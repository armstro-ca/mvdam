    ...
    def check_session(self) -> bool:
        ...
        try:
            if self.expiry >= time.time():
                log.debug("Session expiry (%s) later than current time (%s)", self.expiry, time.time())

                session_update = Connect(
                    "refresh", client_id=None, client_secret=None, refresh_token=self.session["refresh_token"]
                ).action()
                ...
                log.debug("Session expiry (%s) earlier than current time (%s)", self.expiry, time.time())

                session_update = Connect(
                    "auth", username=None, password=None, client_id=None, client_secret=None, grant_type="password"
                ).action()
